...
{
    public partial class HighClosingValue : System.Web.UI.Page
    {
        ...
        private void GenerateReport()
        {
            ...
            string fileName = HttpContext.Current.Server.MapPath(".") + "\\Template\\ClosingValue.xlsx";
            myExcelWorkbook = myExcelWorkbooks.Open(fileName, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue);


            GetStockDetails objStock = new GetStockDetails();

            objStock.PostingDate = DateTime.ParseExact(txtAsOfDate.Text.Trim().ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture);
            ...
            if (txtLocation.Text.Trim().Length > 0)
            {
                ...
                fileName = HttpContext.Current.Server.MapPath(".") + "\\Template\\ClosingValueOne.xlsx";
                myExcelWorkbook = myExcelWorkbooks.Open(fileName, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue, misValue);
                Excel1.Worksheet xlSheet = (Excel1.Worksheet)myExcelWorkbook.Sheets[1];

                objStock.Location = location;

                dtStock = objStock.GetHighestClosingValues();
            ...
            string filePath = Server.MapPath(".") + "\\Reports\\ClosingValue _"+ddlType.Text.ToString() + DateTime.Now.Day + "-" + DateTime.Now.Month + "-" + DateTime.Now.Year + "_" + rnd.Next() + ".xlsx";
        ...
        private void WriteToExcel(DataTable dtStock, Excel1.Worksheet myExcelWorksheet, string location)
        {
            ...
            switch (location)
            {
                ...
                case "0424": storeName = "Matalan Middle East Al Qasar Mall (" + location + ")";
                    break;
                case "0425": storeName = "Matalan Middle East Mecca Mall (" + location + ")";
                    break;
                case "0426": storeName = "Matalan Middle East  (" + location + ")";
                    break;
            ...
            for (int i = 0; i < dtStock.Rows.Count; i++, j++)
            {
                ...
                myExcelWorksheet.get_Range("C" + j, misValue).Formula = (null != dtStock.Rows[i]["Value"]) ? dtStock.Rows[i]["Value"].ToString() : "0";
